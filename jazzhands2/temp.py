...
while camera.isOpened():
    ...
    if isBgCaptured == 1:
        ...
        if length >= 2:
            ...
            res1, res2 = contours[ci[0]], contours[ci[1]]
            hull1, hull2 = cv2.convexHull(res1), cv2.convexHull(res2)
            mask = np.zeros(img.shape, np.uint8)
            cv2.drawContours(mask, [res1, res2], -1, (0, 255, 0), 2)
            cv2.drawContours(mask, [hull1, hull2], -1, (0, 0, 255), 3)

            isFinishCal, cnt = calculateFingers([res1, res2], mask)
            if triggerSwitch and isFinishCal:
                if cnt == 0:
                    prev = 0
                elif cnt == 1 and cnt != prev:
                    prev = 1
                    filename = 'jazzhands/c.wav'
                    wave_obj = sa.WaveObject.from_wave_file(filename)
                    play_obj = wave_obj.play()
                    #play_obj.wait_done()  # Wait until sound has finished playing
                elif cnt == 2 and cnt != prev:
                    prev = 2
                    filename = 'jazzhands/d.wav'
                    wave_obj = sa.WaveObject.from_wave_file(filename)
                    play_obj = wave_obj.play()
                    #play_obj.wait_done()  # Wait until sound has finished playing
                elif cnt == 3 and cnt != prev:
                    prev = 3
                    filename = 'jazzhands/e.wav'
                    wave_obj = sa.WaveObject.from_wave_file(filename)
                    play_obj = wave_obj.play()
                    #play_obj.wait_done()  # Wait until sound has finished playing
